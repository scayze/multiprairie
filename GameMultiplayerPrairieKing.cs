...
namespace MultiPlayerPrairie
{
	...
	public class GameMultiplayerPrairieKing : IMinigame
	{
		...
		public void NETskipLevel()
        {
            ...
            {
				PK_EnemyKilled enemyKilled = new PK_EnemyKilled();
			...
			OnCompleteLevel();
			PK_CompleteLevel completeLevel = new PK_CompleteLevel();
		...
		{
			CowboyBullet bullet = new CowboyBullet(this, position, motion, damage);
			...
			//NET Spawn Bullet
			PK_BulletSpawn mBullet = new PK_BulletSpawn();
		...
		{
			CowboyBullet bullet = new CowboyBullet(this, position, direction, damage);
        ...
        {
			PK_PlayerMove message = new PK_PlayerMove();
		...
		public class CowboyMonster
		{
			...
			public CowboyMonster(GameMultiplayerPrairieKing game, int which, Point position)
			{
				this.gameInstance = game;
				...
				if(gameInstance.isHost)
                {
					...
					PK_EnemySpawn message = new PK_EnemySpawn();
					message.id = this.id;
					message.which = which;
					message.position = position;
					message.isDracula = (which == -2);
					message.isOutlaw = (which == -1);
					...
					case 6:
						{
							...
							if (type == 2)
							{
								...
								{
									if (gameInstance.monsters[i].type == 6 && gameInstance.monsters[i].special && gameInstance.monsters[i].position.Intersects(attemptedPosition))
									{
										//Net EnemyKilled
										PK_EnemyKilled message = new PK_EnemyKilled();
		...
		public class Dracula : CowboyMonster
		{
			...
			public Dracula(GameMultiplayerPrairieKing game)
				: base(game, -2, new Point(8 * TileSize, 8 * TileSize))
			{
		...
		public class Outlaw : CowboyMonster
		{
            ...
            public override void onDeath()
            {
				if (gameInstance.isHost)
				{
					gameInstance.powerups.Add(new CowboyPowerup(gameInstance, (gameInstance.world == 0) ? (-1) : (-2), new Point(8 * TileSize, 10 * TileSize), 9999999));
				}
		...
		public const int POWERUP_COIN = 0;

		public const int POWERUP_NICKEL = 1;
		...
		public class Player
		{
			...
			public List<int> shootingDirections = new List<int>();
		...
		public List<CowboyMonster> monsters = new List<CowboyMonster>();

		protected HashSet<Vector2> _borderTiles = new HashSet<Vector2>();

		public Vector2 playerPosition;

		public Vector2 player2Position = default(Vector2);
		...
		public List<int> playerMovementDirections = new List<int>();

		public List<int> playerShootingDirections = new List<int>();
		...
		public List<CowboyBullet> enemyBullets = new List<CowboyBullet>();
		...
		public List<CowboyPowerup> powerups = new List<CowboyPowerup>();
		...
		public bool LoadGame()
		{
			...
			{
				heldItem = new CowboyPowerup(this, save_data.heldItem.Value, Point.Zero, 9999);
			}
		...
		public void addGuts(Point position, int whichGuts)
		{
			switch (whichGuts)
			{
				case 0:
		...
		public void killOutlaw()
		{
			if (isHost)
            {
				powerups.Add(new CowboyPowerup(this, (world == 0) ? (-1) : (-2), new Point(8 * TileSize, 10 * TileSize), 9999999));
            }
		...
		public void updateBullets(GameTime time)
		{
							...
							if (monsters[k].takeDamage(bullets[m].damage))
							{
								...
								if (shootoutLevel)
								{
									...
									if (whichWave == 12 && monsters[k].type == -2)
									{
										...
										if (isHost)
                                        {
											powerups.Add(new CowboyPowerup(this, -3, new Point(8 * TileSize, 10 * TileSize), 9999999));
										}
								...
								//NET EnemyKilled
								PK_EnemyKilled mEnemyKilled = new PK_EnemyKilled();
			...
			//NET Player death
			PK_PlayerDeath message = new PK_PlayerDeath();
			message.id = Game1.player.UniqueMultiplayerID;
		...
		public void playerDie()
		{
			...
			else
			{
				...
				//Disable collision between palyers
				/*
		...
		public bool tick(GameTime time)
		{
			...
			if (motionPause <= 0)
			{
				...
				if (deathTimer <= 0f && playerMovementDirections.Count > 0 && !scrollingMap)
				{
					...
					for (int i7 = powerups.Count - 1; i7 >= 0; i7--)
					{
						...
						{
							//NET Pickup Powerup
							PK_PowerupPickup message = new PK_PowerupPickup();
							message.id = powerups[i7].id;
							message.which = powerups[i7].which;
					...
					{
						PK_StartLevelTransition message = new PK_StartLevelTransition();
							...
							if (!playerBoundingBox.Intersects(shoppingCarpetNoPickup) && playerBoundingBox.Intersects(storeItems.ElementAt(i8).Key) && coins >= getPriceForItem(storeItems.ElementAt(i8).Value))
							{
								...
								switch (itemToHold)
								{
									...
									case 9:
										...
										break;
									case 10:
										heldItem = new CowboyPowerup(this, 10, Point.Zero, 9999);
										break;
								}
							}
						}
				...
				else if (deathTimer <= 0f && !waitingForPlayerToMoveDownAMap && !shopping && !shootoutLevel)
				{
					...
					for (int p = 0; p < 4; p++)
					{
						...
						if (spawnQueue[p][0].X == 1 || spawnQueue[p][0].X == 4)
						{
							...
							{
								if(isHost)
									monsters.Add(new CowboyMonster(this,spawnQueue[p][0].X, new Point((int)tile.X * TileSize, (int)tile.Y * TileSize)));
							...
							case 0:
								{
									for (int x = 7; x < 10; x++)
									{
										...
										{
											if (isHost)
												monsters.Add(new CowboyMonster(this,spawnQueue[p].First().X, new Point(x * TileSize, 0)));
							...
							case 1:
								{
									for (int y = 7; y < 10; y++)
									{
										...
										{
											if (isHost)
												monsters.Add(new CowboyMonster(this,spawnQueue[p].First().X, new Point(15 * TileSize, y * TileSize)));
							...
							case 2:
								{
									for (int x2 = 7; x2 < 10; x2++)
									{
										...
										{
											if (isHost)
												monsters.Add(new CowboyMonster(this,spawnQueue[p].First().X, new Point(x2 * TileSize, 15 * TileSize)));
							...
							case 3:
								{
									for (int y2 = 7; y2 < 10; y2++)
									{
										...
										{
											if (isHost)
												monsters.Add(new CowboyMonster(this,spawnQueue[p].First().X, new Point(0, y2 * TileSize)));
					...
					if (waveTimer <= 0 && monsters.Count == 0 && isSpawnQueueEmpty())
					{
						...
						if(isHost)
                        {
							PK_CompleteLevel message = new PK_CompleteLevel();
			...
			if(isHost)
            {
				PK_EnemyPositions message = new PK_EnemyPositions();
		...
		public bool isMapTilePassable(int tileType)
		{
		...
		public bool isMapTilePassableForMonsters(int tileType)
		{
		...
		*/
		...
		public void unload()
		{
			...
			PK_ExitGame message = new PK_ExitGame();
